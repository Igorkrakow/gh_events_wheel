...
def run():
    ...
    checkpoint = f'{conf.CHECK_POINTS_PATH}delete_event'
    table = f'{conf.SCHEMA_PATH}.gh_events_delete_silver'
    spark.conf.set("spark.sql.shuffle.partitions", 32)
    df = (
        spark.readStream
        .table(source_table)
    )
    silver_delete_event_df = (df
    ...
    silver_delete_event_clean_df = silver_delete_event_df.dropDuplicates(["id"])
    query = (
        silver_delete_event_clean_df.writeStream
